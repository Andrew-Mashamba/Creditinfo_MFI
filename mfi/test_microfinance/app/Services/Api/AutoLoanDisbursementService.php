...
class AutoLoanDisbursementService
{
    ...
    public function createAndDisburseLoan($clientNumber, $amount, $tenure = null)
    {
        ...
        try {
            ...
            Log::info('✅ MFI Bank Account Retrieved', [
                'transaction_id' => $transactionId,
    ...
    /**
     * Get MFI bank account for disbursement
     */
    private function getSaccosBankAccount()
    {
        Log::info('🏦 Fetching MFI bank account');
        ...
        if (!$bankAccount || !$bankAccount->account_number) {
            throw new Exception('MFI bank account not found (id=1). Please configure bank settings.');
        }
    ...
    private function processNBCTransfer($client, $loan, $bankAccount, $netAmount, $transactionId)
    {
        ...
        try {
            ...
            $transferData = [
                'from_account' => $bankAccount->account_number,  // MFI bank account at NBC
        ...
        if ($client->email) {
            try {
                ...
                $emailData = [
                    ...
                    'company_name' => config('app.name', 'MFI Core System')
                ];
