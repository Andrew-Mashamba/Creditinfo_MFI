...
class SavingsFullReport extends Component
{
    ...
    public $dateFrom = '';
    public $dateTo = '';
    public $reportType = 'detailed'; // summary, detailed, comparative
    ...
    public $quickSearch = ''; // Search for member name, account number, member number
    public $minAmount = ''; // Minimum balance filter
    public $maxAmount = ''; // Maximum balance filter
    public $datePreset = 'this_month'; // Quick date range presets
    public $showAdvancedFilters = false; // Toggle advanced filters visibility
    ...
    public function applyDatePreset($preset = null)
    {
        if (!$preset) {
        ...
        switch ($preset) {
            ...
            case 'last_90_days':
                ...
                break;
            case 'all_time':
                $this->dateFrom = Carbon::now()->subYears(10)->format('Y-m-d');
                $this->dateTo = Carbon::now()->format('Y-m-d');
                break;
            default:
                $this->dateFrom = Carbon::now()->startOfMonth()->format('Y-m-d');
    ...
    public function updated($propertyName)
    {
        if (in_array($propertyName, [
            ...
            'quickSearch', 'minAmount', 'maxAmount'
        ])) {
            $this->resetPage();
            $this->loadReport();
        }

        if ($propertyName === 'datePreset') {
            $this->applyDatePreset($this->datePreset);
        }
    }
    ...
    protected function buildBaseQuery()
    {
        ...
        if ($this->minAmount !== '' && $this->minAmount !== null) {
            $query->where('balance', '>=', $this->minAmount);
        }

        if ($this->maxAmount !== '' && $this->maxAmount !== null) {
            $query->where('balance', '<=', $this->maxAmount);
        }
    ...
    private function exportToExcel($account, $transactions, $openingBalance, $closingBalance, $totalCredits, $totalDebits)
    {
        ...
        return Excel::download(new class($account, $transactions, $openingBalance, $closingBalance, $totalCredits, $totalDebits, $this->dateFrom, $this->dateTo) implements FromArray, WithHeadings, WithStyles, WithTitle {
            ...
            public function headings(): array {
                return [
                    'MFI CORE SYSTEM - SAVINGS ACCOUNT STATEMENT',
                    '',
                    'Account Information',
