...
class LoanRepaymentSchedule extends LivewireDatatable
{
        ...
        return [
          ...
          //  Column::name('loan_id')->label('loan Id'),
            ...
            Column::name('principle')
                ...
                // Column::callback(['id'],function ($loan_id){

                //     $loan = LoansModel::where('id', $loan_id)->first();

                //     $amount= loans_schedules::where('loan_id', $loan->id)->latest()->value('installment');
                //   return number_format($amount,1);

                // })->label('Installment Amount')->searchable(),
            ...
            // return Excel::download(new LoanScheduleReport($id ),'LoanScheduleReport.xlsx');
                 $html = '<div class="w-8 h-8 bg-red-600 rounded-full cursor-pointer" wire:click="download('.$id.')">';
