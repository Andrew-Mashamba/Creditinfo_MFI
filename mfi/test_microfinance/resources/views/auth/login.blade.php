<x-authentication-layout>
        ...
        <div>
            ...
            <div class="relative">
                ...
                <input id="email" name="email" type="email" autocomplete="email" required
                    class="form-input block w-full pl-12 pr-4 py-3 text-white placeholder-red-200 rounded-xl transition-all duration-300 @error('email') border-red-400 @enderror"
        ...
        <div>
            ...
            <div class="relative">
                ...
                <input id="password" name="password" type="password" autocomplete="current-password" required
                    class="form-input block w-full pl-12 pr-12 py-3 text-white placeholder-red-200 rounded-xl transition-all duration-300 @error('password') border-red-400 @enderror"
                    placeholder="Enter your password">
                <button type="button" id="togglePassword" class="absolute inset-y-0 right-0 pr-4 flex items-center">
                    <svg class="h-5 w-5 text-red-200 hover:text-white transition-colors duration-200" id="eyeIcon" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="flex items-center justify-between">
            <div class="flex items-center">
                <input id="remember_me" name="remember" type="checkbox"
                    class="h-4 w-4 text-brand-500 focus:ring-brand-500 border-red-300 rounded bg-transparent">
            ...
            @if (Route::has('password.request'))
                <div class="text-sm">
                    <a href="{{ route('password.request') }}" class="font-medium text-red-200 hover:text-white transition-colors duration-200">
                        Forgot password?
                    </a>
                </div>
            @endif
    ...
    <div class="mt-8">
        <div class="relative">
            <div class="absolute inset-0 flex items-center">
                <div class="w-full border-t border-red-200 border-opacity-30"></div>
            </div>
            <div class="relative flex justify-center text-sm">
                <span class="px-4 glass-dark rounded-full text-red-200">
                    Security Notice
                </span>
            </div>
        </div>
        <div class="mt-6 text-center">
            <div class="flex items-center justify-center space-x-2 p-4 glass-dark rounded-xl">
                ...
                <span class="text-gray-500 text-sm">All activities are monitored. Unauthorized access is prohibited.</span>
            </div>
        </div>
    </div>
    ...
    <script nonce="{{ csp_nonce() }}">
        ...
        passwordInput.addEventListener('input', function() {
            ...
            const strengthColors = ['', 'bg-red-500', 'bg-orange-500', 'bg-yellow-500', 'bg-red-500', 'bg-green-500'];
        ...
        inputs.forEach(input => {
            input.addEventListener('focus', function() {
                this.parentElement.classList.add('ring-2', 'ring-red-400', 'ring-opacity-50');
            });
            
            input.addEventListener('blur', function() {
                this.parentElement.classList.remove('ring-2', 'ring-red-400', 'ring-opacity-50');
            });
        });
