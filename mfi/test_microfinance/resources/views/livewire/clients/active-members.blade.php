    ...
    <div class="bg-white rounded-lg shadow-md">
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 space-y-4 md:space-y-0">
            <div class="flex items-center space-x-4">
                <h2 class="text-lg font-semibold text-gray-800">Active Clients</h2>
       ...
       {{-- <div x-show="$wire.showFilters" 
        ...
        <div x-show="$wire.showExportOptions" 
            ...
            <div class="mt-2 text-sm text-gray-500">
                ...
                Export will include {{ $this->totalRecords }} active clients with currently visible columns
            </div>
        </div>--}}
        ...
        <div class="mb-6 flex flex-col md:flex-row justify-between items-start md:items-center space-y-4 md:space-y-0">
            <div class="relative w-full md:w-96">
                <input wire:model.live.debounce.300ms="search" type="text" placeholder="Search active clients..." 
                        ...
                        <tr wire:key="client-{{ $member->id }}" class="hover:bg-gray-50">                            
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                <div class="flex items-center space-x-3">
                                    ...
                                    <button wire:click="editClient({{ $member->id }})" class="text-red-900 hover:text-red-900" title="Edit">
                                        <i class="fas fa-edit"></i>
                                    </button>
                                    <button wire:click="$emit('deleteClient', {{ $member->id }})" class="text-red-600 hover:text-red-900" title="Delete">
                ...
                <div class="bg-white rounded-2xl w-full max-w-4xl shadow-lg mt-8 mb-8">
                    ...
                    <div class="sticky top-0 bg-white border-b border-gray-200 p-4 rounded-t-2xl z-10">
                        <div class="flex justify-between items-center">
                            <h3 class="text-xl font-semibold text-gray-900">All Client Data</h3>
                            <div class="flex items-center space-x-2">
            ...
            <div class="bg-white rounded-2xl w-full max-w-4xl shadow-none p-0 overflow-hidden">
                <div class="p-4">
                    <div class="flex justify-between items-center mb-4">
                        <h3 class="text-xl font-semibold text-gray-900">Edit Active Client</h3>
                    ...
                    <div class="h-[90vh] overflow-y-auto">
                        <form wire:submit.prevent="saveClient" class="space-y-4">
                            ...
                            <div class="mb-6">
                                <div class="flex items-center space-x-4">
                                    <div class="w-24 h-24 rounded-full overflow-hidden bg-gray-100">
                                        @if($tempPhotoUrl)
                                            <img src="{{ $tempPhotoUrl }}" alt="Client photo" class="w-full h-full object-cover">
                                        @else
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                @foreach($editingClient->getAttributes() as $key => $value)
                                    @if($key !== 'id' && $key !== 'created_at' && $key !== 'updated_at' && $key !== 'deleted_at' && $key !== 'branch_id' && $key !== 'bills' && $key !== 'loans' && $key !== 'accounts' && $key !== 'photo_url' && $key !== 'institution_id')
                                        ...
                                        <div class="col-span-2 sm:col-span-1">
                                            ...
                                            @if($fieldConfig['type'] === 'textarea')
                                                <textarea wire:model="editingClient.{{ $key }}" 
                                            ...
                                            @elseif($fieldConfig['type'] === 'select')
                                                <select wire:model="editingClient.{{ $key }}" 
                                                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-green-500 focus:ring-green-500 sm:text-sm {{ $disabledClass }}"
                                            ...
                                            @else
                                                <input type="{{ $fieldConfig['type'] }}" 
                                                    wire:model="editingClient.{{ $key }}"
                                                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-green-500 focus:ring-green-500 sm:text-sm {{ $disabledClass }}"
                                                    {{ $isRequired }} {{ $isDisabled }}>
                                            @endif
                                            @error("editingClient.$key") 
    ...
    @if($showActionModal && $actionClient)
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
            <div class="bg-white rounded-xl shadow-xl w-full max-w-md p-6 relative">
                <div class="flex justify-between items-center mb-4">
                    <h3 class="text-lg font-semibold text-gray-900">Client Action</h3>
                ...
                <div class="mb-4">
                    <div class="text-sm text-gray-700 mb-1">Client:</div>
