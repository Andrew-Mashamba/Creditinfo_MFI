<div class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 backdrop-blur-sm p-4" x-data="{ activeTab: 'personal' }">
    <div class="bg-white rounded-2xl w-[95vw] max-w-[1600px] max-h-[95vh] shadow-2xl overflow-hidden flex flex-col">
        <!-- Modal Header -->
        <div class="px-8 py-6 border-b border-gray-200 bg-gradient-to-r from-red-50 to-indigo-50 flex-shrink-0">
        ...
        <div class="px-8 pt-6 border-b border-gray-200 bg-gray-50 flex-shrink-0 overflow-x-auto">
            <nav class="flex gap-2 min-w-max" role="tablist">
                <button @click="activeTab = 'personal'" :class="activeTab === 'personal' ? 'bg-white text-red-600 border-red-600 shadow-sm' : 'bg-transparent text-gray-600 hover:bg-gray-100 border-transparent'"
                ...
                <button @click="activeTab = 'contact'" :class="activeTab === 'contact' ? 'bg-white text-red-600 border-red-600 shadow-sm' : 'bg-transparent text-gray-600 hover:bg-gray-100 border-transparent'"
                    class="px-4 py-3 rounded-t-lg font-medium text-sm border-b-2 transition-all whitespace-nowrap flex items-center gap-2">
                    <i class="fas fa-phone"></i> Contact
                </button>
                <button @click="activeTab = 'identification'" :class="activeTab === 'identification' ? 'bg-white text-red-600 border-red-600 shadow-sm' : 'bg-transparent text-gray-600 hover:bg-gray-100 border-transparent'"
                    class="px-4 py-3 rounded-t-lg font-medium text-sm border-b-2 transition-all whitespace-nowrap flex items-center gap-2">
                    <i class="fas fa-id-card"></i> Identification
                </button>
                <button @click="activeTab = 'employment'" :class="activeTab === 'employment' ? 'bg-white text-red-600 border-red-600 shadow-sm' : 'bg-transparent text-gray-600 hover:bg-gray-100 border-transparent'"
                    class="px-4 py-3 rounded-t-lg font-medium text-sm border-b-2 transition-all whitespace-nowrap flex items-center gap-2">
                    <i class="fas fa-briefcase"></i> Employment & Finance
                </button>
                <button @click="activeTab = 'business'" :class="activeTab === 'business' ? 'bg-white text-red-600 border-red-600 shadow-sm' : 'bg-transparent text-gray-600 hover:bg-gray-100 border-transparent'"
                    class="px-4 py-3 rounded-t-lg font-medium text-sm border-b-2 transition-all whitespace-nowrap flex items-center gap-2">
                    <i class="fas fa-building"></i> Business Info
                </button>
                <button @click="activeTab = 'address'" :class="activeTab === 'address' ? 'bg-white text-red-600 border-red-600 shadow-sm' : 'bg-transparent text-gray-600 hover:bg-gray-100 border-transparent'"
                    class="px-4 py-3 rounded-t-lg font-medium text-sm border-b-2 transition-all whitespace-nowrap flex items-center gap-2">
                    <i class="fas fa-map-marker-alt"></i> Address
                </button>
                <button @click="activeTab = 'guarantor'" :class="activeTab === 'guarantor' ? 'bg-white text-red-600 border-red-600 shadow-sm' : 'bg-transparent text-gray-600 hover:bg-gray-100 border-transparent'"
                    class="px-4 py-3 rounded-t-lg font-medium text-sm border-b-2 transition-all whitespace-nowrap flex items-center gap-2">
                    <i class="fas fa-user-shield"></i> Guarantor
                </button>
                <button @click="activeTab = 'membership'" :class="activeTab === 'membership' ? 'bg-white text-red-600 border-red-600 shadow-sm' : 'bg-transparent text-gray-600 hover:bg-gray-100 border-transparent'"
                    class="px-4 py-3 rounded-t-lg font-medium text-sm border-b-2 transition-all whitespace-nowrap flex items-center gap-2">
                    <i class="fas fa-users"></i> Membership
                </button>
                <button @click="activeTab = 'other'" :class="activeTab === 'other' ? 'bg-white text-red-600 border-red-600 shadow-sm' : 'bg-transparent text-gray-600 hover:bg-gray-100 border-transparent'"
                    class="px-4 py-3 rounded-t-lg font-medium text-sm border-b-2 transition-all whitespace-nowrap flex items-center gap-2">
        ...
        <div class="flex-1 overflow-y-auto p-8 bg-white">
            <form wire:submit.prevent="saveClient" id="editClientForm">
                @php
                    $fieldGroups = [
                                ...
                                <div class="{{ in_array($field, ['address', 'notes']) ? 'md:col-span-2 lg:col-span-3 xl:col-span-4' : '' }}">
                                    ...
                                    @else
                                        <input type="text" wire:model="editingMember.{{ $field }}"
                                            class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-transparent {{ in_array($field, ['client_number', 'account_number', 'phone_number']) ? 'bg-gray-50' : '' }}"
        ...
        <div class="px-8 py-6 border-t border-gray-200 bg-gray-50 flex justify-between items-center flex-shrink-0">
            ...
            <div class="flex gap-3">
                ...
                <button type="submit" form="editClientForm"
                    class="px-6 py-3 text-sm font-medium text-white bg-red-600 rounded-xl hover:bg-red-700 transition-colors shadow-lg hover:shadow-xl">
