<div class="container mx-auto p-6 space-y-8">
    ...
    <div class="mb-2">
            ...
            @foreach($steps as $index => $label)
                <div class="flex-1 flex items-center group">
                    <div class="w-10 h-10 flex items-center justify-center rounded-full text-sm font-semibold 
                        {{ $currentStep >= ($index + 1) ? 'bg-red-600 text-white' : 'bg-gray-300 text-gray-600' }}">
                        {{ $index + 1 }}
                    </div>
                    @if($index < count($steps) - 1)
                        <div class="flex-1 h-1 mx-2 {{ $currentStep > ($index + 1) ? 'bg-red-600' : 'bg-gray-300' }}"></div>
                    @endif
                </div>
            @endforeach
        </div>
        <div class="flex justify-between mt-2 text-sm text-gray-500 font-medium">
            @foreach($steps as $index => $label)
                <div class="flex-1 text-center {{ $currentStep === ($index + 1) ? 'text-red-900 font-semibold' : '' }}">
                    {{ $label }}
                </div>
            @endforeach
        </div>
    ...
    @if($currentStep === 1)
        <div class="bg-white p-6 rounded-xl shadow ring-1 ring-gray-100">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <select wire:model="selectedBankAccountId" id="selectedBankAccountId"
                            class="w-full border-gray-300 rounded-md shadow-sm focus:ring-red-900 focus:border-red-900">
            ...
            <div class="mb-4">
                <a href="#" wire:click.prevent="downloadTemplate" class="text-red-900 hover:underline text-sm">Download Template</a>
            </div>

            <div>
                <input type="file" wire:model="uploadFile"
                    class="block w-full text-sm border-gray-300 rounded-md shadow-sm focus:ring-red-900 focus:border-red-900 mb-2" accept=".csv,.xlsx,.xls">
                @error('uploadFile') <div class="text-red-600 text-sm">{{ $message }}</div> @enderror
                <div wire:loading wire:target="uploadFile" class="text-red-900 text-sm">Uploading...</div>

                @if($uploadFile)
                    <div class="mt-4">
                        <button wire:click="processFilePreview"
                            class="bg-red-600 text-white px-4 py-2 rounded-md hover:bg-red-600 shadow focus:ring-2 focus:ring-red-900"
    ...
    @if($currentStep === 3)
        <div class="bg-white p-6 rounded-xl shadow flex flex-col items-center ring-1 ring-gray-100">
            ...
            <div class="w-full bg-gray-200 rounded-full h-4 mb-4 overflow-hidden">
                <div class="bg-red-600 h-full transition-all duration-300 ease-in-out" style="width: {{ $uploadProgress }}%"></div>
            </div>
