<div class="bg-white rounded-lg shadow-lg p-6">
    <h3 class="text-xl font-semibold text-gray-800 mb-6">
        <i class="fas fa-chart-line mr-2 text-red-600"></i>Comparative Analysis
    </h3>

    @if(isset($comparativeData) && isset($comparativeData['current_period']) && isset($comparativeData['previous_period']))
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            <!-- Current Period -->
            <div class="bg-gradient-to-br from-red-50 to-red-100 p-6 rounded-lg border-l-4 border-red-600 shadow">
                ...
                <div class="space-y-3">
                    <div class="flex justify-between items-center">
                        <span class="text-red-700 font-medium">Accounts:</span>
                        <span class="font-bold text-red-900 text-lg">{{ number_format($comparativeData['current_period']->account_count) }}</span>
                    </div>
                    <div class="flex justify-between items-center">
                        <span class="text-red-700 font-medium">Members:</span>
                        <span class="font-bold text-red-900 text-lg">{{ number_format($comparativeData['current_period']->member_count) }}</span>
                    </div>
                    <div class="flex justify-between items-center pt-2 border-t border-red-200">
                        <span class="text-red-700 font-medium">Total Balance:</span>
                        <span class="font-bold text-red-900 text-lg">TZS {{ number_format($comparativeData['current_period']->total_balance, 2) }}</span>
                    </div>
                    <div class="flex justify-between items-center">
                        <span class="text-red-700 font-medium">Average Balance:</span>
                        <span class="font-bold text-red-900 text-lg">TZS {{ number_format($comparativeData['current_period']->avg_balance, 2) }}</span>
                    </div>
                </div>
            </div>
