<div class="bg-white rounded-2xl shadow-lg border border-gray-100 p-6 mt-4">
    ...
    <div class="mb-6 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <form wire:submit.prevent="markAttendance" class="flex flex-col md:flex-row gap-2 items-center">
            <select wire:model.defer="leader_id" class="w-48 px-3 py-2 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-transparent">
                <option value="">Select Leader</option>
                @foreach($availableLeaders as $leader)
                    <option value="{{ $leader->id }}">{{ $leader->full_name }}</option>
                @endforeach
            </select>
            <select wire:model.defer="status" class="w-40 px-3 py-2 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-transparent">
                <option value="present">Present</option>
                <option value="absent">Absent</option>
                <option value="excused">Excused</option>
            </select>
            <input type="number" wire:model.defer="stipend_amount" placeholder="Stipend (optional)" class="w-32 px-3 py-2 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-transparent" step="0.01" min="0" />
            <input type="text" wire:model.defer="notes" placeholder="Notes (optional)" class="w-64 px-3 py-2 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-transparent" />
            <button type="submit" class="px-4 py-2 bg-red-600 text-white rounded hover:bg-red-800">Mark</button>
        </form>
        <input type="text" wire:model.debounce.300ms="search" placeholder="Search attendance..." class="w-64 px-3 py-2 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-transparent" />
    </div>
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                @forelse($attendance as $record)
                    <tr>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <button wire:click="toggleStipendPaid({{ $record->id }})" class="text-red-600 hover:text-red-900 font-semibold">Toggle Paid</button>
                        </td>
                    </tr>
                @empty
                    <tr>
