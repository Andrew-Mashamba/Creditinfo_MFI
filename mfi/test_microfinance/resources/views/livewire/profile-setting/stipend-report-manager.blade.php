<div class="bg-white rounded-2xl shadow-lg border border-gray-100 p-6 mt-4">
    ...
    <div class="mb-4 flex gap-8">
        <div class="bg-red-50 rounded-lg p-4 flex-1">
            <div class="text-xs text-red-700">Total Paid</div>
            <div class="text-2xl font-bold text-red-900">{{ number_format($totalPaid, 2) }}</div>
            <div class="text-xs text-red-700">({{ $countPaid }} records)</div>
        </div>
    ...
    <div class="overflow-x-auto">
        <div class="mb-2 flex items-center gap-2">
            <input type="checkbox" wire:model="selectAll" class="form-checkbox h-4 w-4 text-red-600">
            <span class="text-sm">Select All</span>
            <button wire:click="markSelectedAsPaid" class="ml-4 px-4 py-2 bg-red-600 text-white rounded hover:bg-red-800 disabled:opacity-50" @if(count($selectedRecords) == 0) disabled @endif>Mark Selected as Paid</button>
        ...
        <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50">
                <tr>
                    <th class="px-2 py-3"><input type="checkbox" wire:model="selectAll" class="form-checkbox h-4 w-4 text-red-600"></th>
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                @forelse($records as $record)
                    <tr>
                        <td class="px-2 py-4"><input type="checkbox" wire:model="selectedRecords" value="{{ $record->id }}" class="form-checkbox h-4 w-4 text-red-600"></td>
