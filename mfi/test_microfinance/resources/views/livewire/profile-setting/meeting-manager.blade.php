<div class="bg-white rounded-2xl shadow-lg border border-gray-100 p-6 mt-4">
    <div class="flex items-center justify-between mb-4">
        ...
        <button wire:click="resetForm" class="px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-800 transition-colors">Add Meeting</button>
    </div>
    @if (session()->has('message'))
        <div class="mb-4 p-3 bg-green-100 text-green-800 rounded">{{ session('message') }}</div>
    @endif
    <div class="flex items-center justify-between mb-4">
        <input type="text" wire:model.debounce.300ms="search" placeholder="Search by title, agenda, or location..." class="w-64 px-3 py-2 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-transparent" />
        <div>
                ...
                @forelse($meetings as $meeting)
                    <tr>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <button wire:click="editMeeting({{ $meeting->id }})" class="text-red-600 hover:text-red-900 font-semibold mr-2">Edit</button>
        ...
        <div class="bg-white rounded-lg shadow-lg p-8 w-full max-w-md">
            ...
            <form wire:submit.prevent="{{ $editMode ? 'updateMeeting' : 'createMeeting' }}">
                <div class="mb-4">
                    ...
                    <input type="text" wire:model.defer="title" class="w-full px-3 py-2 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-transparent">
                ...
                <div class="mb-4">
                    ...
                    <textarea wire:model.defer="agenda" class="w-full px-3 py-2 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-transparent"></textarea>
                    @error('agenda') <span class="text-red-600 text-xs">{{ $message }}</span> @enderror
                </div>
                <div class="mb-4">
                    <label class="block text-sm font-medium text-gray-700 mb-1">Date & Time</label>
                    <input type="datetime-local" wire:model.defer="meeting_date" class="w-full px-3 py-2 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-transparent">
                    @error('meeting_date') <span class="text-red-600 text-xs">{{ $message }}</span> @enderror
                </div>
                <div class="mb-4">
                    <label class="block text-sm font-medium text-gray-700 mb-1">Location</label>
                    <input type="text" wire:model.defer="location" class="w-full px-3 py-2 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-transparent">
                    @error('location') <span class="text-red-600 text-xs">{{ $message }}</span> @enderror
                </div>
                <div class="mb-4">
                    <label class="block text-sm font-medium text-gray-700 mb-1">Notes</label>
                    <textarea wire:model.defer="notes" class="w-full px-3 py-2 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-transparent"></textarea>
