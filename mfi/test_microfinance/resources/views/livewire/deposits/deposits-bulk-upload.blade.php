<div class="container mx-auto p-6">
    ...
    <div class="mb-6 flex items-center space-x-4">
        <div class="flex items-center">
            ...
            <span class="ml-2 {{ $currentStep >= 1 ? 'font-bold text-red-900' : 'text-gray-500' }}">Upload</span>
        </div>
        ...
        <div class="flex items-center">
            ...
            <span class="ml-2 {{ $currentStep >= 2 ? 'font-bold text-red-900' : 'text-gray-500' }}">Preview</span>
        </div>
        <div class="h-1 w-8 bg-gray-300"></div>
        <div class="flex items-center">
            ...
            <span class="ml-2 {{ $currentStep >= 3 ? 'font-bold text-red-900' : 'text-gray-500' }}">Process</span>
        </div>
        <div class="h-1 w-8 bg-gray-300"></div>
        <div class="flex items-center">
            ...
            <span class="ml-2 {{ $currentStep == 4 ? 'font-bold text-red-900' : 'text-gray-500' }}">Results</span>
        </div>
    </div>
    ...
    @if($currentStep === 1)
        <div class="bg-white p-6 rounded shadow">
            <h2 class="text-xl font-semibold mb-4">Bulk Deposits Upload</h2>
            
            {{-- Bank Account Selection --}}
            <div class="mb-6 p-4 bg-gray-50 rounded">
                <h3 class="text-lg font-medium mb-4">Bank Account Configuration</h3>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <label for="selectedBankAccountId" class="block text-sm font-medium text-gray-700 mb-2">
                            Bank Account *
                        </label>
                        <select wire:model="selectedBankAccountId" id="selectedBankAccountId" class="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-500">
                    ...
                    <div>
                        <label for="selectedInternalMirrorAccountId" class="block text-sm font-medium text-gray-700 mb-2">
                            Internal Mirror Account *
                        </label>
                        <select wire:model="selectedInternalMirrorAccountId" id="selectedInternalMirrorAccountId" class="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-500">
    ...
    @if($currentStep === 2)
        <div class="bg-white p-6 rounded shadow">
            ...
            <div class="mb-4 text-gray-600">
                <p>Showing first {{ count($previewData) }} rows. Please review and fix any errors before proceeding.</p>
                <p class="mt-2 text-sm">
                    <strong>Automatic Values:</strong> Payment Method: Bank, Date: Today, Time: Current Time, 
                    Narration: "Bulk deposits - [Member ID] - [Account Number]", Depositor: "Unknown"
                </p>
            </div>
                    ...
                    <tbody>                        
                        @foreach($processingResults as $result)                        
                            <tr>
                                ...
                                <td class="border p-2">{{ $result['account'] ?? '' }}</td>
                                <td class="border p-2">{{ $result['amount'] ?? '' }}</td>
                                <td class="border p-2">{{ $result['bank'] ?? $result['payment_method'] ?? '' }}</td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
