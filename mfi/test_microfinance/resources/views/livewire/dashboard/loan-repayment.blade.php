<div class="w-full p-2">
    ...
    <div class="flex items-center justify-between mb-4 p-3 bg-gray-50 rounded">
        <h2 class="text-lg font-semibold text-red-900">Loan Repayment</h2>
        @if($currentStep > 1)
            <button wire:click="newPayment" class="px-3 py-1.5 bg-red-600 text-white text-sm rounded hover:bg-red-800">
    ...
    <div class="mb-4 p-3 bg-white rounded shadow-sm">
        <div class="flex items-center justify-between">
            ...
            @foreach($steps as $step => $label)
                <div class="flex items-center {{ $currentStep >= $step ? 'text-red-900' : 'text-gray-400' }}">
                    <div class="rounded-full h-8 w-8 flex items-center justify-center border {{ $currentStep >= $step ? 'border-red-900 bg-red-600 text-white' : 'border-gray-300' }} text-xs font-semibold">
                        {{ $step }}
                    </div>
                    <span class="ml-1 text-xs hidden sm:inline">{{ $label }}</span>
                </div>
                @if($step < 4)
                    <div class="flex-1 border-t {{ $currentStep > $step ? 'border-red-900' : 'border-gray-300' }} mx-2"></div>
                @endif
            @endforeach
        </div>
    </div>
    ...
    @if (session()->has('info'))
        <div class="bg-red-50 border-l-4 border-red-500 p-3 mb-3">
            <p class="text-sm text-red-700">{{ session('info') }}</p>
        </div>
    @endif
    ...
    @if($currentStep == 1)
        <div class="bg-white rounded shadow-sm p-4">
            <h3 class="text-sm font-semibold text-red-900 mb-3">Search for Loan</h3>
            
            <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
                <div>
                    <label class="block text-xs text-gray-600 mb-1">Search By</label>
                    <select wire:model="searchType" class="w-full px-2 py-1.5 text-sm border border-gray-300 rounded focus:border-red-900 focus:outline-none">
                ...
                <div>
                    ...
                    <input type="text" wire:model="searchValue" 
                           class="w-full px-2 py-1.5 text-sm border border-gray-300 rounded focus:border-red-900 focus:outline-none"
                ...
                <div class="flex items-end">
                    <button wire:click="searchLoan" 
                            class="w-full px-3 py-1.5 bg-red-600 text-white text-sm rounded hover:bg-red-800">
        ...
        @if($showLoanSelection && count($availableLoans) > 0)
            <div class="bg-white rounded shadow-sm p-4 mt-3">
                <h3 class="text-sm font-semibold text-red-900 mb-2">Select Loan</h3>
                                ...
                                <tr>
                                    ...
                                    <td class="px-3 py-2">
                                        <button wire:click="selectLoan('{{ $loan['loan_id'] }}')" 
                                                class="px-3 py-1 bg-red-600 text-white text-xs rounded hover:bg-red-800">
                                            Select
                                        </button>
                                    </td>
                                </tr>
        ...
        <div class="space-y-3">
            ...
            <div class="bg-white rounded shadow-sm p-4">
                <div class="flex justify-between items-center mb-3">
                    <h3 class="text-sm font-semibold text-red-900">Loan Information</h3>
                    <div class="space-x-2">
                        <button wire:click="togglePaymentHistory" class="text-xs text-red-900 hover:underline">
                            History
                        </button>
                        <button wire:click="toggleSchedule" class="text-xs text-red-900 hover:underline">
                            Schedule
                        </button>
                    </div>
                </div>
            ...
            <div class="bg-white rounded shadow-sm p-4">
                <h3 class="text-sm font-semibold text-red-900 mb-3">Outstanding Balance</h3>
                <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
                    ...
                    <div class="border-l-4 border-red-900 pl-3">
                        <label class="text-xs text-gray-600">Total</label>
                        <p class="text-lg font-bold text-red-900">{{ number_format($outstandingBalance['total'] ?? 0, 0) }}</p>
                    </div>
                </div>
                
                <button wire:click="calculateEarlySettlement" class="mt-3 text-xs text-red-900 hover:underline">
            ...
            @if($showPaymentHistory && count($paymentHistory) > 0)
                <div class="bg-white rounded shadow-sm p-4">
                    <h3 class="text-sm font-semibold text-red-900 mb-3">Recent Payments</h3>
                    <div class="overflow-x-auto">
                        <table class="min-w-full text-xs">
            ...
            @if($showSchedule && count($repaymentSchedule) > 0)
                <div class="bg-white rounded shadow-sm p-4">
                    <h3 class="text-sm font-semibold text-red-900 mb-3">Repayment Schedule</h3>
                    <div class="overflow-x-auto">
                        <table class="min-w-full text-xs">
                            <thead class="bg-gray-50">
                                <tr>
            ...
            {{-- Payment Form --}}
            <div class="bg-white rounded shadow-sm p-4">
                <h3 class="text-sm font-semibold text-red-900 mb-3">Payment Information</h3>
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="number" wire:model="paymentAmount" 
                                   class="w-full pl-10 pr-2 py-1.5 text-sm border border-gray-300 rounded focus:border-red-900 focus:outline-none"
                                   placeholder="0.00" step="0.01" min="100">
                        </div>
                        @error('paymentAmount') <span class="text-xs text-red-500">{{ $message }}</span> @enderror
                        
                        @if($paymentAmount)
                            <button wire:click="previewAllocation" class="mt-1 text-xs text-red-900 hover:underline">
                                Preview Allocation
                            </button>
                        @endif
                    </div>
                ...
                <div class="mt-4 flex justify-between">
                    ...
                    <button wire:click="nextStep" 
                            class="px-4 py-1.5 bg-red-600 text-white text-sm rounded hover:bg-red-800"
    ...
    @if($currentStep == 3 && $selectedLoanId)
        <div class="bg-white rounded shadow-sm p-4">
            <h3 class="text-sm font-semibold text-red-900 mb-3">Confirm Payment</h3>
            ...
            <div class="grid grid-cols-2 gap-3 p-3 bg-gray-50 rounded mb-3">
                ...
                <div>
                    ...
                    <p class="text-lg font-bold text-red-900">{{ number_format($paymentAmount ?? 0, 0) }}</p>
                </div>
                <div>
        ...
        <div class="bg-white rounded-lg shadow-sm p-6">
            ...
            <div class="bg-gray-50 p-4 rounded-lg mb-4">
                <div id="receipt-content" class="bg-white border-2 border-gray-300 rounded p-6 max-w-md mx-auto">
                    ...
                    <div class="text-center border-b-2 border-gray-800 pb-3 mb-4">
                        <h1 class="text-base font-bold uppercase">MFI CORE SYSTEM</h1>
                    ...
                    <div class="bg-gray-100 p-3 rounded mb-4">
                        <div class="text-center">
                            ...
                            <p class="text-2xl font-bold text-red-900">TZS {{ number_format($receipt['amount_paid'] ?? 0, 0) }}</p>
                        </div>
                    </div>
            ...
            <div class="flex justify-center space-x-3">
                <button onclick="printLoanReceipt()" 
                        class="inline-flex items-center px-4 py-2 bg-red-600 text-white text-sm font-medium rounded-md hover:bg-red-800 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500">
