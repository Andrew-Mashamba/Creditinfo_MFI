<div wire:click.prefetch="toggle('{{ $index }}')"
     class="@if($column['hidden']) relative table-cell h-12 w-3 bg-red-100 hover:bg-red-300 overflow-none align-top group @else hidden @endif"
     style="min-width:12px; max-width:12px"
     >
     <button class="relative h-12 w-3 focus:outline-none">
         <span
             class="w-32 hidden group-hover:inline-block absolute z-10 top-0 left-0 ml-3 bg-red-300 font-medium leading-4 text-xs text-left text-red-700 tracking-wider transform uppercase focus:outline-none">
...
<div class="@if($column['hidden']) hidden @else relative h-12 overflow-hidden align-top flex table-cell @endif" @include('datatables::style-width')>
    ...
    @if($column['sortable'])
        <button wire:click="sort('{{ $index }}')"
            ...
            <span class="inline text-xs text-red-400">
            @if($sort === $index)
                @if($direction)
    ...
    @if ($column['hideable'])
        <button wire:click.prefetch="toggle('{{ $index }}')"
            ...
            <x-icons.arrow-circle-left class="h-3 w-3 text-gray-300 hover:text-red-400" />
        </button>
    @endif
</div>
