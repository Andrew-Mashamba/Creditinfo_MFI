<div>
    ...
    <div class="relative">
        <div class="flex items-center justify-between mb-1">
                ...
                @if($this->searchableColumns()->count())
                    <div class="flex rounded-lg w-96 shadow-sm">
                        <div class="relative flex-grow focus-within:z-10">
                            ...
                            <input wire:model.debounce.500ms="search" class="block w-full py-3 pl-10 text-sm border-gray-300 leading-4 rounded-md shadow-sm focus:border-red-300 focus:ring focus:ring-red-200 focus:ring-opacity-50 focus:outline-none" placeholder="{{__('Search in')}} {{ $this->searchableColumns()->map->label->join(', ') }}" type="text" />
            ...
            @if($this->activeFilters)
                <span class="text-xl text-red-400 uppercase">@lang('Filter active')</span>
            @endif
            ...
            <div class="flex flex-wrap items-center space-x-1">
                ...
                @if(count($this->massActionsOptions))
                    <div class="flex items-center space-x-2 bg-gray-50 p-2 rounded-lg border border-gray-200">
                        <label for="datatables_mass_actions" class="text-sm font-medium text-gray-700">{{ __('With selected') }}:</label>
                        <div class="relative">
                            <select wire:model="massActionOption" 
                                class="block w-48 pl-3 pr-10 py-2 text-sm border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-red-500 bg-white" 
                        ...
                        <button
                            ...
                            class="inline-flex items-center px-4 py-2 text-sm font-medium text-white bg-green-600 border border-transparent rounded-md shadow-sm hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 transition-colors duration-200"
                            type="submit"
                            title="Submit"
                        >
                            <span>{{ __('Apply') }}</span>
            ...
            <div class="p-2 grid grid-cols-8 gap-2">
                @foreach($this->columns as $index => $column)
                    @if ($column['hideable'])
                        ...
                        {{ $column['hidden'] ? 'bg-red-100 hover:bg-red-300 text-red-600' : 'bg-red-500 hover:bg-red-800' }}">
                            {{ $column['label'] }}
                        </button>
                    @endif
                @endforeach
        ...
        <div wire:loading.class="opacity-50" class="rounded-lg @unless($complex || $this->hidePagination) rounded-b-none @endunless shadow-lg bg-white max-w-screen overflow-x-scroll border-2 @if($this->activeFilters) border-red-500 @else border-transparent @endif @if($complex) rounded-b-none border-b-0 @endif">
            <div>
                <div class="table min-w-full align-middle">
                    ...
                    @endunless
                    <div class="table-row bg-red-100 divide-x divide-red-200">
                        @foreach($this->columns as $index => $column)
                            @if($column['hidden'])
                                @if($hideable === 'inline')
                                    <div class="table-cell w-5 overflow-hidden align-top bg-red-100"></div>
                                @endif
                            @elseif($column['type'] === 'checkbox')
        ...
        @unless($this->hidePagination)
            <div class="max-w-screen bg-white @unless($complex) rounded-b-lg @endunless border-4 border-t-0 border-b-0 @if($this->activeFilters) border-red-500 @else border-transparent @endif">
                <div class="items-center justify-between p-2 sm:flex">
                    {{-- check if there is any data --}}
                    @if(count($this->results))
                        <div class="flex items-center my-2 sm:my-0">
                            <select name="perPage" class="block w-full py-2 pl-3 pr-10 mt-1 text-base border-gray-300 form-select leading-6 focus:outline-none focus:shadow-outline-red focus:border-red-300 sm:text-sm sm:leading-5" wire:model="perPage">
    ...
    @if($complex)
        <div class="bg-gray-50 px-4 py-4 rounded-b-lg rounded-t-none shadow-lg border-2 @if($this->activeFilters) border-red-500 @else border-transparent @endif @if($complex) border-t-0 @endif">
