        ...
        <div wire:key="{{ $key }}">
            ...
            @elseif($rule['type'] === 'group')
                <div x-data="{
                    ...
                    x-on:drop="drop"
                    class="p-4 space-y-4 bg-red-500 bg-opacity-10 rounded-lg text-gray-{{ strlen($parentIndex) > 6 ? 1 : 9 }}00 border border-red-400"
                >
                    <span class="flex justify-center space-x-4">
                        <button wire:click="addRule('{{ collect(explode('.', $key))->join(".content.") . ".content" }}')" class="flex items-center space-x-2 px-3 py-2 border border-red-400 rounded-md bg-white text-red-500 text-xs leading-4 font-medium uppercase tracking-wider hover:bg-red-200 focus:outline-none">ADD RULE</button>
                        <button wire:click="addGroup('{{ collect(explode('.', $key))->join(".content.") . ".content" }}')" class="flex items-center space-x-2 px-3 py-2 border border-red-400 rounded-md bg-white text-red-500 text-xs leading-4 font-medium uppercase tracking-wider hover:bg-red-200 focus:outline-none">ADD GROUP</button>
                    </span>
                            ...
                            @if(count($rule['content']) > 1)
                                <div class="mr-8">
                                    ...
                                    <select
                                        ...
                                        class="w-24 text-sm leading-4 block rounded-md border-gray-300 shadow-sm focus:border-red-300 focus:ring focus:ring-red-200 focus:ring-opacity-50"
                                    >
