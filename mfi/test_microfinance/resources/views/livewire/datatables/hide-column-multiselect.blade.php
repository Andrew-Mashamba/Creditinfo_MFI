<div x-data="{ show: false }" class="flex flex-col items-center">
    <div class="flex flex-col items-center relative">
        <button x-on:click="show = !show" class="px-3 py-2 border border-red-400 rounded-md bg-white text-red-500 text-xs leading-4 font-medium uppercase tracking-wider hover:bg-red-200 focus:outline-none">
            ...
            <div class="flex flex-col w-full">
                @foreach($this->columns as $index => $column)
                    @if ($column['hideable'] !== false)
                        <div>
                            <div class="@unless($column['hidden']) hidden @endif cursor-pointer w-full border-gray-800 border-b bg-gray-700 text-gray-500 hover:bg-red-600 hover:text-white" wire:click="toggle({{$index}})">
