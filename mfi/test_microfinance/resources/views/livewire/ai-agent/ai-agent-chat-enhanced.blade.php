...
class="overflow-hidden chat-container" style="height: 90vh; min-height: 600px;">
    ...
    <div x-show="showUploadModal" 
        ...
        <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div x-transition:enter="transition ease-out duration-300"
                ...
                <!-- Upload Area -->
                <div class="mb-4">
                    <h3 class="text-lg font-semibold text-red-900 mb-4">Upload Files</h3>
                    
                    <!-- Drag & Drop Zone -->
                    <div @dragover.prevent="dragActive = true"
                         @dragleave.prevent="dragActive = false"
                         @drop.prevent="dragActive = false; $wire.handleFileDrop($event)"
                         :class="{'border-red-500 bg-red-50': dragActive}"
                        ...
                        <label for="file-upload" class="cursor-pointer">
                            <span class="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md text-white bg-red-600 hover:bg-red-800 transition-colors">
                                Browse Files
                            </span>
                ...
                <div class="flex justify-end space-x-3">
                    ...
                    <button wire:click="processUploadedFiles" 
                            class="px-4 py-2 text-sm font-medium text-white bg-red-600 rounded-md hover:bg-red-800 transition-colors">
                        Upload & Analyze
                    </button>
                </div>
            </div>
    ...
    <div class="flex overflow-hidden bg-white rounded-2xl shadow-lg border border-gray-100 chat-main" style="height: 85vh; min-height: 550px;">
        ...
        <div class="w-64 bg-white shadow-xl border-r border-gray-200 flex flex-col sidebar">
            <!-- Header -->
            <div class="p-4 border-b border-gray-100 bg-gradient-to-r from-red-900 to-red-800">
                <div class="flex items-center space-x-3">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-gray-500">MFI Intelligence</p>
                    </div>
                </div>
            </div>
            ...
            <div class="p-3 border-b border-gray-100">
                <div class="grid grid-cols-2 gap-2">
                    <button wire:click="newConversation" 
                            class="flex items-center justify-center p-2 text-xs font-medium text-red-900 bg-red-50 rounded-lg hover:bg-red-100 transition-colors">
            ...
            <div class="p-3 border-b border-gray-100 bg-red-50/50">
                <h3 class="text-xs font-semibold text-red-900 uppercase mb-2">Active Context Files</h3>
            ...
            <div class="flex-1 overflow-y-auto p-6 space-y-6" id="chat-messages">
                @if(empty($messages))
                    ...
                    <div class="max-w-3xl mx-auto text-center py-12">
                        <div class="w-20 h-20 bg-gradient-to-br from-red-900 to-red-700 rounded-2xl flex items-center justify-center mx-auto mb-6 shadow-lg">
                            <svg class="w-12 h-12 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z"></path>
                            </svg>
                        </div>
                        <h3 class="text-2xl font-bold text-gray-900 mb-2">Welcome to Zona AI Assistant</h3>
                        <p class="text-gray-600 mb-8">Your intelligent MFI management companion</p>
                        
                        <!-- Quick Start Suggestions -->
                        <div class="grid grid-cols-2 gap-3 max-w-2xl mx-auto">
                            <button wire:click="sendQuickMessage('Show me account summary')" 
                                    class="p-3 text-left bg-white border border-gray-200 rounded-lg hover:border-red-300 hover:shadow-md transition-all group">
                        ...
                        <div class="flex items-start space-x-3 {{ $message['sender'] === 'user' ? 'justify-end' : '' }}">
                            @if($message['sender'] === 'ai')
                                <div class="flex-shrink-0">
                                    <div class="w-10 h-10 bg-gradient-to-br from-red-900 to-red-700 rounded-xl flex items-center justify-center shadow-md">
                                        <svg class="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z"></path>
                                        </svg>
                                    </div>
                            ...
                            <div class="flex-1 max-w-3xl {{ $message['sender'] === 'user' ? 'order-first' : '' }}">
                                <div class="rounded-2xl p-4 shadow-sm {{ $message['sender'] === 'user' ? 'bg-red-600 text-white' : 'bg-white border border-gray-200' }}">
                                    @if($message['sender'] === 'user' && !empty($message['attachments']))
                                    <div class="flex flex-wrap gap-2 mb-2">
                                        @foreach($message['attachments'] as $attachment)
                                        <span class="inline-flex items-center px-2 py-1 rounded-md text-xs font-medium bg-red-800 text-gray-500">
                ...
                @if($isLoading)
                    <div class="flex items-start space-x-3">
                        <div class="flex-shrink-0">
                            <div class="w-10 h-10 bg-gradient-to-br from-red-900 to-red-700 rounded-xl flex items-center justify-center shadow-md animate-pulse">
                                <svg class="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z"></path>
                                </svg>
                            </div>
            ...
            <div class="bg-white border-t border-gray-200 p-4">
                <form wire:submit.prevent="sendMessage" class="max-w-4xl mx-auto">
                    ...
                    <div class="mb-3 flex flex-wrap gap-2">
                        @foreach($attachedFiles as $file)
                        <div class="inline-flex items-center px-3 py-1 rounded-lg bg-red-50 border border-red-200">
                            <svg class="w-4 h-4 text-red-600 mr-2" fill="currentColor" viewBox="0 0 20 20">
                                <path fill-rule="evenodd" d="M4 4a2 2 0 012-2h8l4 4v10a2 2 0 01-2 2H6a2 2 0 01-2-2V4z"/>
                            </svg>
                            <span class="text-sm text-red-900">{{ $file['name'] }}</span>
                            <button type="button" 
                    ...
                    <div class="relative flex items-end space-x-3">
                        ...
                        <div class="flex-1 relative">
                            <textarea 
                                wire:model="message" 
                                rows="1" 
                                class="w-full border border-gray-300 rounded-xl px-4 py-3 pr-12 focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-transparent resize-none text-gray-900 placeholder-gray-500 transition-all duration-200"
                        ...
                        <button 
                            type="submit" 
                            class="p-3 bg-red-600 hover:bg-red-800 disabled:bg-gray-400 text-white rounded-xl transition-all duration-200 shadow-md hover:shadow-lg disabled:shadow-none"
                            {{ $isLoading ? 'disabled' : '' }}
                        >
                            @if($isLoading)
