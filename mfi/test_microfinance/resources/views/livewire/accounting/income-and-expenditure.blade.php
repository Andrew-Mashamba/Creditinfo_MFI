...
<div class="p-4 shadow-md sm:rounded-lg bg-gray-100 flex">
    ...
    <div class="w-2/3 mx-auto bg-white p-6 shadow-md">
        ...
        <div class="text-center">
            <h2 class="text-lg font-bold uppercase text-xs">MFI Management System LTD</h2>
        ...
        <div class="mt-6">
            <table class="w-full text-left border-collapse text-sm">
                ...
                <tr>
                    <td colspan="2" class="bg-red-50 font-bold px-4 py-2 border"></td>
                    <td class="bg-red-50 font-bold px-4 py-2 border"></td>
                    <td class="bg-red-50 font-bold px-4 py-2 border text-xs">ESTIMATES 75%</td>
                    <td class="bg-red-50 font-bold px-4 py-2 border text-xs">ACTUAL 75%</td>
                    <td class="bg-red-50 font-bold px-4 py-2 border text-xs">DIFFERENCE</td>
                    <td class="bg-red-50 font-bold px-4 py-2 border text-xs">%</td>
                    <td class="bg-red-50 font-bold px-4 py-2 border text-xs">BUDGET 2024</td>
                </tr>
                <tr>
                    <td colspan="2" class="bg-red-50 font-bold px-4 py-2 border text-xs">INCOMES</td>
                    <td class="bg-red-50 font-bold px-4 py-2 border text-xs">CODE</td>
                    <td class="bg-red-50 font-bold px-4 py-2 border text-xs">YEAR 2024</td>
                    <td class="bg-red-50 font-bold px-4 py-2 border text-xs"></td>
                    <td class="bg-red-50 font-bold px-4 py-2 border text-xs">Tshs</td>
                    <td class="bg-red-50 font-bold px-4 py-2 border text-xs">(%)</td>
                    <td class="bg-red-50 font-bold px-4 py-2 border text-xs">Tshs</td>
                </tr>
                ...
                <tr>
                    <td colspan="2" class="bg-red-50 font-bold px-4 py-2 border text-xs">ZIADA KABLA YA KODI</td>
                    <td class="bg-red-50 font-bold px-4 py-2 border text-right text-xs"></td>
                    <td class="bg-red-50 font-bold px-4 py-2 border  text-right text-xs">{{ number_format($total_income - $total_expenses, 2) }}</td>
                    <td class="bg-red-50 font-bold px-4 py-2 border  text-right text-xs"> {{ number_format($total_income - $total_expenses, 2) }}</td>
                    <td class="bg-red-50 font-bold px-4 py-2 border  text-right text-xs"> {{ number_format($total_income - $total_expenses, 2) }} </td>
                    <td class="bg-red-50 font-bold px-4 py-2 border  text-right text-xs">{{ number_format($total_income - $total_expenses, 2) }}</td>
                    <td class="bg-red-50 font-bold px-4 py-2 border text-right text-xs">{{ number_format($total_income - $total_expenses, 2) }}</td>
                ...
                <tr>
                    <td colspan="2" class="bg-red-50 font-bold px-4 py-2 border text-xs">Gharama za Kodi</td>
                    <td class="bg-red-50 font-bold px-4 py-2 border"></td>
                    <td class="bg-red-50 font-bold px-4 py-2 border text-right text-xs">{{ number_format(($total_income - $total_expenses)*30/100, 2) }}</td>
                    <td class="bg-red-50 font-bold px-4 py-2 border text-right text-xs">{{ number_format(($total_income - $total_expenses)*30/100, 2) }}</td>
                    <td class="bg-red-50 font-bold px-4 py-2 border text-right text-xs">{{ number_format(($total_income - $total_expenses)*30/100, 2) }}</td>
                    <td class="bg-red-50 font-bold px-4 py-2 border text-right text-xs">{{ number_format(($total_income - $total_expenses)*30/100, 2) }}</td>
                    <td class="bg-red-50 font-bold px-4 py-2 border text-right text-xs">{{ number_format(($total_income - $total_expenses)*30/100, 2) }}</td>
                ...
                <tr>
                    <td colspan="2" class="bg-red-50 font-bold px-4 py-2 border text-xs">ZIADA BAADA KODI</td>
                    <td class="bg-red-50 font-bold px-4 py-2 border"></td>
                    <td class="bg-red-50 font-bold px-4 py-2 border text-right text-xs">{{ number_format(($total_income - $total_expenses)-($total_income - $total_expenses)*30/100, 2) }}</td>
                    <td class="bg-red-50 font-bold px-4 py-2 border text-right text-xs">{{ number_format(($total_income - $total_expenses)-($total_income - $total_expenses)*30/100, 2) }}</td>
                    <td class="bg-red-50 font-bold px-4 py-2 border text-right text-xs">{{ number_format(($total_income - $total_expenses)-($total_income - $total_expenses)*30/100, 2) }}</td>
                    <td class="bg-red-50 font-bold px-4 py-2 border text-right text-xs">{{ number_format(($total_income - $total_expenses)-($total_income - $total_expenses)*30/100, 2) }}</td>
                    <td class="bg-red-50 font-bold px-4 py-2 border text-right text-xs">{{ number_format(($total_income - $total_expenses)-($total_income - $total_expenses)*30/100, 2) }}</td>
    ...
    <div class="w-1/3 mx-auto bg-white p-6 shadow-md">
        ...
        @if($this->selected_sub_category_code)
            ...
            <table class="w-full text-left border-collapse text-sm">
                ...
                <!-- MTIRIRIKO WA FEDHA TOKANA NA SHUGHULI ZA UENDESHAJI Section -->
                <tr>
                    <td colspan="1" class="bg-red-50 font-bold px-4 py-2 border">{{ucwords(str_replace('_', ' ', $this->category_name))}}</td>
                    <td class="bg-red-50 font-bold px-4 py-2 border">DEBIT</td>
                    <td class="bg-red-50 font-bold px-4 py-2 border">CREDIT</td>
                </tr>
                @php
                ...
                @foreach($second_level as $income_account)
                    ...
                    <tr>
                        <td class="px-4 py-2 border">
                            <a wire:click="setSubCode2({{ $income_account->sub_category_code }},'{{$income_account->account_name}}')" class="cursor-pointer text-red-600 hover:text-red-800 text-sm">
        ...
        @if($this->selected_sub_category_code2)
            <table class="w-full text-left border-collapse text-sm">
                ...
                <!-- MTIRIRIKO WA FEDHA TOKANA NA SHUGHULI ZA UENDESHAJI Section -->
                <tr>
                    <td colspan="1" class="bg-red-50 font-bold px-4 py-2 border">{{ucwords(str_replace('_', ' ', $this->category_name2))}} - {{$this->selected_sub_category_code2}}</td>
                    <td class="bg-red-50 font-bold px-4 py-2 border">DEBIT</td>
                    <td class="bg-red-50 font-bold px-4 py-2 border">CREDIT</td>
                </tr>
                @php
                ...
                @foreach($second_level2 as $income_account)
                    ...
                    <tr>
                        <td class="px-4 py-2 border">
                            <a wire:click="setSubCode3({{ $income_account->sub_category_code }},'{{$income_account->account_name}}')" class="cursor-pointer text-red-600 hover:text-red-800 text-sm">
                                {{ ucwords(str_replace('_', ' ', $income_account->account_name)) }}
                            </a>
                        </td>
                        <td class="px-4 py-2 border text-right">
