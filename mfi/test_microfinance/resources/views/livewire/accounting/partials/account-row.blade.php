@php
    ...
    // Professional color scheme - only red-900 and red for accents
...
<tr class="{{ $rowBackground }} hover:bg-red-50 transition-colors {{ $level > 0 ? 'border-l-2 border-gray-300' : '' }}">
        ...
        <div class="flex items-center" style="padding-left: {{ 24 + $indentPixels }}px;">
            ...
            @if($hasChildren)
                <button wire:click="toggleAccount('{{ $account->account_number }}')" 
                    class="mr-2 p-1 rounded text-gray-500 hover:text-red-900 hover:bg-gray-100 focus:outline-none transition-all">
                    @if($isExpanded)
            ...
            <div class="flex items-center">
                ...
                <svg class="w-4 h-4 mr-2 {{ $accountType == 'LIABILITY' || $accountType == 'EXPENSE' ? 'text-red-600' : 'text-red-900' }}" 
    ...
    <td class="px-6 py-3 whitespace-nowrap text-center">
        <div class="flex items-center justify-center space-x-1">
            <button wire:click="viewLedger('{{ $account->account_number }}')" 
                class="p-1 text-red-900 hover:text-red-700 hover:bg-red-50 rounded transition-all focus:outline-none" 
                title="View Ledger">
