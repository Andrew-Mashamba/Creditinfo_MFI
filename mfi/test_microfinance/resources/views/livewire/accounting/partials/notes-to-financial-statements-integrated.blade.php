@php
    ...
    $companyName = $companyName ?? 'MFI INSTITUTION';
    $selectedYear = $selectedYear ?? date('Y');
    $corporateInfo = $corporateInfo ?? [
        'name' => 'MFI Institution',
        'nature' => 'Savings and Credit Cooperative Society',
        'registration' => 'REG/MFI/2020',
    ...
    $significantEstimates = $significantEstimates ?? [];
    ...
    $comparisonYears = $comparisonYears ?? [$selectedYear, $selectedYear - 1, $selectedYear - 2];
    $reportingDate = $reportingDate ?? date('d F Y');
    $showAccountDetail = $showAccountDetail ?? false;
    $selectedAccountForDetail = $selectedAccountForDetail ?? ['name' => '', 'number' => ''];
    $accountTransactions = $accountTransactions ?? [];
@endphp

<div class="bg-white rounded-lg shadow-sm">
    <!-- Header Section -->
    <div class="bg-gradient-to-r from-red-900 to-red-800 text-white px-4 py-3 rounded-t-lg">
    ...
    <div class="px-4 py-2 bg-gray-50 border-b border-gray-200">
        <div class="flex justify-between items-center">
            <div class="flex items-center space-x-2">
                ...
                <select wire:model="selectedYear" class="text-xs border-gray-300 rounded-md shadow-sm focus:border-red-500 focus:ring-red-500">
    ...
    <div class="p-4 space-y-6">
        ...
        <div class="border border-gray-200 rounded-lg overflow-hidden">
            <button wire:click="toggleNote('corporate')" 
                    class="w-full px-3 py-2 bg-red-50 hover:bg-red-100 transition-colors duration-200 flex justify-between items-center">
                <span class="text-sm font-semibold text-red-900">1. CORPORATE INFORMATION</span>
                <svg class="w-4 h-4 text-red-600 transform transition-transform {{ in_array('corporate', $expandedNotes) ? 'rotate-180' : '' }}" 
        ...
        <div class="border border-gray-200 rounded-lg overflow-hidden">
            <button wire:click="toggleNote('compliance')" 
                    class="w-full px-3 py-2 bg-red-50 hover:bg-red-100 transition-colors duration-200 flex justify-between items-center">
                <span class="text-sm font-semibold text-red-900">2. STATEMENT OF COMPLIANCE</span>
                <svg class="w-4 h-4 text-red-600 transform transition-transform {{ in_array('compliance', $expandedNotes) ? 'rotate-180' : '' }}" 
                     fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path>
                </svg>
            </button>
        ...
        <div class="border border-gray-200 rounded-lg overflow-hidden">
            <button wire:click="toggleNote('policies')" 
                    class="w-full px-3 py-2 bg-red-50 hover:bg-red-100 transition-colors duration-200 flex justify-between items-center">
                <span class="text-sm font-semibold text-red-900">3. SIGNIFICANT ACCOUNTING POLICIES</span>
                <svg class="w-4 h-4 text-red-600 transform transition-transform {{ in_array('policies', $expandedNotes) ? 'rotate-180' : '' }}" 
                     fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path>
                </svg>
            </button>
            ...
            <div class="px-3 py-2 bg-white">
                ...
                <div class="mb-3 border-b border-gray-100 pb-2 last:border-0">
                    <h4 class="text-xs font-semibold text-red-800 mb-1">{{ $policy['title'] }}</h4>
        ...
        <div class="border border-gray-200 rounded-lg overflow-hidden">
            <button wire:click="toggleNote('estimates')" 
                    class="w-full px-3 py-2 bg-red-50 hover:bg-red-100 transition-colors duration-200 flex justify-between items-center">
                <span class="text-sm font-semibold text-red-900">4. CRITICAL ACCOUNTING ESTIMATES AND JUDGMENTS</span>
                <svg class="w-4 h-4 text-red-600 transform transition-transform {{ in_array('estimates', $expandedNotes) ? 'rotate-180' : '' }}" 
                     fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path>
                </svg>
            </button>
        ...
        <div class="border border-gray-200 rounded-lg overflow-hidden">
            <button wire:click="toggleNote('assets')" 
                    class="w-full px-3 py-2 bg-red-50 hover:bg-red-100 transition-colors duration-200 flex justify-between items-center">
                <span class="text-sm font-semibold text-red-900">5. ASSETS</span>
                <svg class="w-4 h-4 text-red-600 transform transition-transform {{ in_array('assets', $expandedNotes) ? 'rotate-180' : '' }}" 
                     fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path>
                </svg>
            </button>
        ...
        <div class="border border-gray-200 rounded-lg overflow-hidden">
            <button wire:click="toggleNote('liabilities')" 
                    class="w-full px-3 py-2 bg-red-50 hover:bg-red-100 transition-colors duration-200 flex justify-between items-center">
                <span class="text-sm font-semibold text-red-900">6. LIABILITIES</span>
                <svg class="w-4 h-4 text-red-600 transform transition-transform {{ in_array('liabilities', $expandedNotes) ? 'rotate-180' : '' }}" 
                     fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path>
                </svg>
            </button>
        ...
        <div class="border border-gray-200 rounded-lg overflow-hidden">
            <button wire:click="toggleNote('equity')" 
                    class="w-full px-3 py-2 bg-red-50 hover:bg-red-100 transition-colors duration-200 flex justify-between items-center">
                <span class="text-sm font-semibold text-red-900">7. MEMBERS' EQUITY</span>
                <svg class="w-4 h-4 text-red-600 transform transition-transform {{ in_array('equity', $expandedNotes) ? 'rotate-180' : '' }}" 
                     fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path>
                </svg>
            </button>
        ...
        <div class="border border-gray-200 rounded-lg overflow-hidden">
            <button wire:click="toggleNote('income')" 
                    class="w-full px-3 py-2 bg-red-50 hover:bg-red-100 transition-colors duration-200 flex justify-between items-center">
                <span class="text-sm font-semibold text-red-900">8. REVENUE</span>
                <svg class="w-4 h-4 text-red-600 transform transition-transform {{ in_array('income', $expandedNotes) ? 'rotate-180' : '' }}" 
                     fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path>
                </svg>
            </button>
        ...
        <div class="border border-gray-200 rounded-lg overflow-hidden">
            <button wire:click="toggleNote('expenses')" 
                    class="w-full px-3 py-2 bg-red-50 hover:bg-red-100 transition-colors duration-200 flex justify-between items-center">
                <span class="text-sm font-semibold text-red-900">9. OPERATING EXPENSES</span>
                <svg class="w-4 h-4 text-red-600 transform transition-transform {{ in_array('expenses', $expandedNotes) ? 'rotate-180' : '' }}" 
                     fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path>
                </svg>
            </button>
        ...
        <div class="border border-gray-200 rounded-lg overflow-hidden">
            <button wire:click="toggleNote('related')" 
                    class="w-full px-3 py-2 bg-red-50 hover:bg-red-100 transition-colors duration-200 flex justify-between items-center">
                <span class="text-sm font-semibold text-red-900">10. RELATED PARTY TRANSACTIONS</span>
                <svg class="w-4 h-4 text-red-600 transform transition-transform {{ in_array('related', $expandedNotes) ? 'rotate-180' : '' }}" 
                     fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path>
                </svg>
            </button>
        ...
        <div class="border border-gray-200 rounded-lg overflow-hidden">
            <button wire:click="toggleNote('subsequent')" 
                    class="w-full px-3 py-2 bg-red-50 hover:bg-red-100 transition-colors duration-200 flex justify-between items-center">
                <span class="text-sm font-semibold text-red-900">11. EVENTS AFTER THE REPORTING PERIOD</span>
                <svg class="w-4 h-4 text-red-600 transform transition-transform {{ in_array('subsequent', $expandedNotes) ? 'rotate-180' : '' }}" 
                     fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path>
                </svg>
            </button>
        ...
        <div class="border border-gray-200 rounded-lg overflow-hidden">
            <button wire:click="toggleNote('approval')" 
                    class="w-full px-3 py-2 bg-red-50 hover:bg-red-100 transition-colors duration-200 flex justify-between items-center">
                <span class="text-sm font-semibold text-red-900">12. APPROVAL OF FINANCIAL STATEMENTS</span>
                <svg class="w-4 h-4 text-red-600 transform transition-transform {{ in_array('approval', $expandedNotes) ? 'rotate-180' : '' }}" 
                     fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path>
                </svg>
            </button>
    ...
    <div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
        <div class="bg-white rounded-lg shadow-xl max-w-4xl w-full max-h-[80vh] overflow-hidden">
            <div class="bg-gradient-to-r from-red-900 to-red-800 text-white px-4 py-3">
                <div class="flex justify-between items-center">
                    <div>
