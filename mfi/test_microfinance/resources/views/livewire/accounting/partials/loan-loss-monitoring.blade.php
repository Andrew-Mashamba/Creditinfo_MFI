        ...
        @if(count($alerts) > 0)
            <div class="space-y-3">
                @foreach($alerts as $alert)
                    <div class="border-l-4 {{ $alert['type'] === 'danger' ? 'border-red-500 bg-red-50' : ($alert['type'] === 'warning' ? 'border-yellow-500 bg-yellow-50' : 'border-red-500 bg-red-50') }} p-4">
                        <div class="flex">
                            ...
                            <div class="ml-3">
                                <h3 class="text-sm font-medium {{ $alert['type'] === 'danger' ? 'text-red-800' : ($alert['type'] === 'warning' ? 'text-yellow-800' : 'text-red-800') }}">
                                    {{ $alert['title'] }}
                                </h3>
                                <div class="mt-2 text-sm {{ $alert['type'] === 'danger' ? 'text-red-700' : ($alert['type'] === 'warning' ? 'text-yellow-700' : 'text-red-700') }}">
                                    <p>{{ $alert['message'] }}</p>
                                </div>
                                <div class="mt-4">
                                    <div class="-mx-2 -my-1.5 flex">
                                        <button type="button" class="px-2 py-1.5 rounded-md text-sm font-medium {{ $alert['type'] === 'danger' ? 'text-red-800 hover:bg-red-100' : ($alert['type'] === 'warning' ? 'text-yellow-800 hover:bg-yellow-100' : 'text-red-800 hover:bg-red-100') }}">
                                            {{ $alert['action'] }}
                                        </button>
                                    </div>
                                </div>
            ...
            <div class="border rounded-lg p-4">
                ...
                <div class="relative pt-1">
                    ...
                    <div class="overflow-hidden h-2 mb-4 text-xs flex rounded bg-red-200">
                        <div style="width:{{ $utilizationRate }}%" class="shadow-none flex flex-col text-center whitespace-nowrap text-white justify-center bg-red-500"></div>
                    </div>
                </div>
            </div>
