            ...
            <div class="flex flex-wrap items-center gap-3">
                ...
                <div class="flex items-center space-x-2">
                    ...
                    <select wire:model="selectedYear" 
                            class="text-sm border-gray-300 rounded-md shadow-sm focus:border-red-500 focus:ring-red-500">
                ...
                <div class="flex space-x-1 bg-white rounded-lg border border-gray-200 p-1">
                    <button wire:click="switchStatement('dashboard')"
                            class="px-3 py-1.5 text-xs font-medium rounded {{ $currentStatement === 'dashboard' ? 'bg-red-600 text-white' : 'text-gray-700 hover:bg-gray-100' }}">
                        Dashboard
                    </button>
                    <button wire:click="switchStatement('balance_sheet')"
                            class="px-3 py-1.5 text-xs font-medium rounded {{ $currentStatement === 'balance_sheet' ? 'bg-red-600 text-white' : 'text-gray-700 hover:bg-gray-100' }}">
                        Financial Position
                    </button>
                    <button wire:click="switchStatement('income_statement')"
                            class="px-3 py-1.5 text-xs font-medium rounded {{ $currentStatement === 'income_statement' ? 'bg-red-600 text-white' : 'text-gray-700 hover:bg-gray-100' }}">
                        Income Statement
                    </button>
                    <button wire:click="switchStatement('cash_flow')"
                            class="px-3 py-1.5 text-xs font-medium rounded {{ $currentStatement === 'cash_flow' ? 'bg-red-600 text-white' : 'text-gray-700 hover:bg-gray-100' }}">
                        Cash Flows
                    </button>
                    <button wire:click="switchStatement('equity_changes')"
                            class="px-3 py-1.5 text-xs font-medium rounded {{ $currentStatement === 'equity_changes' ? 'bg-red-600 text-white' : 'text-gray-700 hover:bg-gray-100' }}">
                        Equity Changes
                    </button>
                    <button wire:click="switchStatement('trial_balance')"
                            class="px-3 py-1.5 text-xs font-medium rounded {{ $currentStatement === 'trial_balance' ? 'bg-red-600 text-white' : 'text-gray-700 hover:bg-gray-100' }}">
                        Trial Balance
                    </button>
                    <button wire:click="switchStatement('notes')"
                            class="px-3 py-1.5 text-xs font-medium rounded {{ $currentStatement === 'notes' ? 'bg-red-600 text-white' : 'text-gray-700 hover:bg-gray-100' }}">
                        Notes
                    </button>
                ...
                <div class="flex items-center space-x-3">
                    <label class="flex items-center">
                        <input type="checkbox" wire:model="showDetailed" class="rounded border-gray-300 text-red-600">
                        <span class="ml-2 text-sm text-gray-700">Detailed View</span>
                    </label>
                    <label class="flex items-center">
                        <input type="checkbox" wire:model="showComparison" class="rounded border-gray-300 text-red-600">
                        <span class="ml-2 text-sm text-gray-700">Comparison</span>
                    </label>
                    <label class="flex items-center">
                        <input type="checkbox" wire:model="showNotes" class="rounded border-gray-300 text-red-600">
                        <span class="ml-2 text-sm text-gray-700">Notes</span>
                    </label>
                    <label class="flex items-center">
                        <input type="checkbox" wire:model="showRatios" class="rounded border-gray-300 text-red-600">
