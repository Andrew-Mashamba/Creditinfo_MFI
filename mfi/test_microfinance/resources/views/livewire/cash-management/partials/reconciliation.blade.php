    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
        <div class="bg-gradient-to-br from-red-50 to-red-100 rounded-2xl p-6 border border-red-200">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    <p class="text-sm font-medium text-red-700">Reconciled Today</p>
                    <p class="text-2xl font-bold text-red-900">{{ count($reconciledToday ?? []) }}</p>
                </div>
            </div>
        </div>
            ...
            <form wire:submit.prevent="performQuickReconciliation" class="space-y-6">
                ...
                @if($selectedTillForRecon ?? null)
                    <div class="bg-red-50 rounded-2xl p-6 border border-red-200">
                        <h4 class="text-lg font-bold text-red-900 mb-4">Till Details</h4>
                        <div class="grid grid-cols-2 gap-4 text-sm">
                            <div>
                                <p class="text-red-700 font-medium">Opening Balance:</p>
                                <p class="text-red-900 font-bold text-lg">TZS {{ number_format($selectedTillDetails['opening_balance'] ?? 0) }}</p>
                            </div>
                            <div>
                                <p class="text-red-700 font-medium">System Balance:</p>
                                <p class="text-red-900 font-bold text-lg">TZS {{ number_format($selectedTillDetails['system_balance'] ?? 0) }}</p>
                            </div>
                            <div>
                                <p class="text-red-700 font-medium">Transactions:</p>
                                <p class="text-red-900 font-bold">{{ $selectedTillDetails['transaction_count'] ?? 0 }}</p>
                            </div>
                            <div>
                                <p class="text-red-700 font-medium">Last Transaction:</p>
                                <p class="text-red-900">{{ $selectedTillDetails['last_transaction_time'] ?? 'N/A' }}</p>
                            </div>
                        </div>
                    </div>
                @endif
        ...
        <div class="bg-white rounded-3xl shadow-xl border border-gray-100 p-8">
            ...
            <div class="space-y-4">
                        ...
                        <div class="flex justify-between items-center mb-2">
                            ...
                            <div class="text-right">
                                <input type="number" wire:model="denominationCounts.{{ $denomination['id'] }}" 
                                       class="w-20 rounded-lg border-2 border-red-200 shadow-sm focus:border-red-500 focus:ring-2 focus:ring-red-200 transition-all duration-200 p-2 text-center" 
                                       placeholder="0" min="0">
                            </div>
                        </div>
                ...
                <button wire:click="updatePhysicalCountFromDenominations" 
                        class="w-full bg-gradient-to-r from-red-500 to-red-600 text-white py-3 px-6 rounded-xl hover:from-red-600 hover:to-red-700 focus:outline-none focus:ring-4 focus:ring-red-200 transition-all duration-200 font-bold shadow-lg">
                    ...
                    @forelse($reconciliationHistory ?? [] as $reconciliation)
                        <tr class="border-b border-gray-100 hover:bg-gray-50 transition-all duration-200">
                            ...
                            <td class="py-4 px-6 text-right">
                                <p class="font-bold text-red-600">TZS {{ number_format($reconciliation->system_balance) }}</p>
                            </td>
                            <td class="py-4 px-6 text-right">
        ...
        <div class="bg-white rounded-3xl shadow-xl border border-gray-100 p-8">
            ...
            <div class="space-y-4">
                @foreach($reconciliationTrends ?? [] as $trend)
                    <div class="bg-gradient-to-r from-green-50 to-red-50 rounded-2xl p-4 border border-green-200">
                        <div class="flex justify-between items-center mb-2">
