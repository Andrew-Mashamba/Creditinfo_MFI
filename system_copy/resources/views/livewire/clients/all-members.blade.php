        ...
        <div class="px-8 py-6 border-b border-gray-100 bg-gradient-to-r from-blue-50 to-indigo-50">
            <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <h2 class="text-2xl font-bold text-gray-900">All Members</h2>
                        <p class="text-sm text-gray-600 mt-1">View and manage all member records</p>
                    </div>
                </div>
                ...
                <div class="flex items-center gap-4">
                    <div class="px-6 py-3 bg-white rounded-xl shadow-sm border border-gray-200">
                        <div class="flex items-center gap-2">
                            <span class="text-sm font-medium text-gray-600">Total Members</span>
                            <span class="px-3 py-1 text-lg font-bold bg-blue-100 text-blue-700 rounded-lg">
            ...
            <div class="flex flex-col gap-4">
                ...
                <div class="relative">
                    ...
                    <input
                        ...
                        type="text"
                        placeholder="Search members by name, phone, email, member number, account number, NIDA, or branch..."
                        class="block w-full pl-12 pr-4 py-4 text-base border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all shadow-sm hover:shadow-md"
                    >
                    @if($search)
                ...
                <div class="flex flex-wrap items-center gap-3">
                    ...
                    <button wire:click="$set('filters.membership_type', 'Individual')"
                        class="px-4 py-2 rounded-lg text-sm font-medium transition-all {{ $filters['membership_type'] === 'Individual' ? 'bg-blue-100 text-blue-800 border-2 border-blue-300' : 'bg-white text-gray-600 border border-gray-300 hover:bg-gray-50' }}">
                    ...
                    <div class="mt-4 p-6 bg-white rounded-xl border border-gray-200 shadow-sm">
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                            ...
                            <div>
                                ...
                                <select wire:model.live="filters.gender" class="w-full rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                            ...
                            <div>
                                ...
                                <select wire:model.live="filters.marital_status" class="w-full rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                            ...
                            <div>
                                ...
                                <select wire:model.live="filters.nationality" class="w-full rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                            ...
                            <div>
                                ...
                                <input type="date" wire:model.live="filters.start_date" class="w-full rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                            </div>

                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-2">To Date</label>
                                <input type="date" wire:model.live="filters.end_date" class="w-full rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                            </div>
                        </div>
                    </div>
                @endif
                        ...
                        <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-3">
                            @foreach(['account_number', 'client_number', 'branch_number', 'first_name', 'last_name', 'gender', 'phone_number', 'email', 'status', 'membership_type', 'created_at'] as $column)
                                <label class="flex items-center p-3 rounded-lg hover:bg-gray-50 cursor-pointer transition-colors border border-gray-200">
                                    ...
                                    <span class="text-sm text-gray-700">{{ $column === 'client_number' ? 'Member Number' : ($column === 'branch_number' ? 'Branch' : ($column === 'account_number' ? 'Bank Account Number' : ucfirst(str_replace('_', ' ', $column)))) }}</span>
                                </label>
                            @endforeach
                        </div>
                    </div>
            ...
            @if($loading)
                <div class="absolute inset-0 bg-white bg-opacity-90 flex items-center justify-center z-10">
                    <div class="flex flex-col items-center gap-3">
                        ...
                        <span class="text-sm font-medium text-gray-600">Loading members...</span>
                    </div>
                </div>
            @endif
                        ...
                        @foreach($columns as $column => $visible)
                            @if($visible)
                                <th wire:click="sortBy('{{ $column }}')"
                                    ...
                                    <div class="flex items-center gap-2">
                                        <span>{{ $column === 'client_number' ? 'Member Number' : ($column === 'branch_number' ? 'Branch' : ($column === 'account_number' ? 'Bank Account Number' : ucfirst(str_replace('_', ' ', $column)))) }}</span>
                                        @if($sortField === $column)
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse($members as $index => $member)
                        <tr wire:key="member-{{ $member->id }}"
                            class="hover:bg-blue-50 transition-colors {{ $index % 2 === 0 ? 'bg-gray-50' : 'bg-white' }}">
                            @foreach($columns as $column => $visible)
                                @if($visible)
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        ...
                                        @elseif($column === 'membership_type')
                                            <span class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold {{ $member->membership_type === 'Individual' ? 'bg-blue-100 text-blue-800' : 'bg-purple-100 text-purple-800' }}">
                                        ...
                                        @elseif($column === 'client_number' || $column === 'member_number')
                                            <span class="text-sm font-mono font-semibold text-blue-600">{{ $member->$column }}</span>
            ...
            <div class="flex flex-col md:flex-row justify-between items-center gap-4">
                <div class="flex items-center gap-3">
                    ...
                    <select wire:model.live="perPage"
                        class="rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 text-sm font-medium">
